...
>*以股票因子库、指数因子库、股票池以及基金为主体的策略集合，定义完毕后即可在策略执行界面中使用。*

- 策略分为因子导向策略和价格导向策略；分别在因子导向策略和价格导向策略界面中使用；
- 单只基金一般对应两项策略，分别为买入方向与卖出方向策略；
- 策略组合千变万化，组合灵活，可对单一基金定义N只策略，随时切换；
- 为了提升策略的有效性和防范策略的风险，系统对策略定义进行合理性校验；
- 策略的组合条件，均在对应模块定义好，一次定义多次使用；比如因子条件，在因子设计界面中设计；比如自选股，在自选股界面中设计。

![](_assets/images/tactics_define.png)
